...
public class VHSEffect_RLPRO : ScriptableRendererFeature {
	...
	public class VHSEffect_RLPROPass : ScriptableRenderPass {
		...
		void Render(CommandBuffer cmd, ref RenderingData renderingData) {
			...
			if (Random.Range(0, 100 - retroEffect.verticalOffsetFrequency.value) <= 5) {
				if (retroEffect.verticalOffset == 0.0f) {
					RetroEffectMaterial.SetFloat(_OffsetPosY, retroEffect.verticalOffset.value * retroEffect.fade.value);
				}
				if (retroEffect.verticalOffset.value > 0.0f) {
					RetroEffectMaterial.SetFloat(_OffsetPosY, retroEffect.verticalOffset.value * retroEffect.fade.value - Random.Range(0f, retroEffect.verticalOffset.value) * retroEffect.fade.value);
			...
			RetroEffectMaterial.SetFloat(_OffsetDistortion, retroEffect.offsetDistortion.value * retroEffect.fade.value);
			...
			RetroEffectMaterial.SetFloat(_OffsetColor, retroEffect.colorOffset.value * 0.001f * retroEffect.fade.value);

			RetroEffectMaterial.SetFloat(_OffsetNoiseX, Random.Range(-0.4f, 0.4f) * retroEffect.fade.value);
			...
			if (RetroEffectMaterial.HasProperty(_OffsetNoiseY)) {
				...
				RetroEffectMaterial.SetFloat(_OffsetNoiseY, (offsetNoise + Random.Range(-0.03f, 0.03f)) * retroEffect.fade.value);
			}
