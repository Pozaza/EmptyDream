...
public class Noise_RLPRO : ScriptableRendererFeature {
	...
	public class Noise_RLPROPass : ScriptableRenderPass {
		...
		private float _time;
		private RenderTexture texTape = RenderTexture.active;
		...
		void Render(CommandBuffer cmd, ref RenderingData renderingData) {
			...
			if (texTape == null || (texTape.height != Mathf.Min(retroEffect.VerticalResolution.value, screenLinesNum_))) {
				int texHeight = (int)Mathf.Min(retroEffect.VerticalResolution.value, screenLinesNum_) / (Settings.Instance?._quality == Quality.High ? 1 : Settings.Instance?._quality == Quality.Medium ? 2 : 5);
				int texWidth = (int)(texHeight * (float)Screen.width / Screen.height) / (Settings.Instance?._quality == Quality.High ? 1 : Settings.Instance?._quality == Quality.Medium ? 2 : 5);
...
#endif
				texTape = new(texWidth, texHeight, 0) {
