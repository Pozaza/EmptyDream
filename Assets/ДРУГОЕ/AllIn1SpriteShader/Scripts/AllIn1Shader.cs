...
public class AllIn1Shader : MonoBehaviour {
	...
	public void MakeNewMaterial(bool getShaderTypeFromPrefs, string shaderName = "AllIn1SpriteShader") => SetMaterial(AfterSetAction.Clear, getShaderTypeFromPrefs, shaderName);

	public void MakeCopy() => SetMaterial(AfterSetAction.CopyMaterial, false, GetStringFromShaderType());

	private void ResetAllProperties(bool getShaderTypeFromPrefs, string shaderName) => SetMaterial(AfterSetAction.Reset, getShaderTypeFromPrefs, shaderName);
	...
	private string GetStringFromShaderType() {
		...
		else if (shaderTypes == ShaderTypes.Urp2dRenderer)
	...
	public void SaveMaterial() {
		...
		if (PlayerPrefs.HasKey("All1ShaderMaterials"))
			path = PlayerPrefs.GetString("All1ShaderMaterials") + "/";
		else
			PlayerPrefs.SetString("All1ShaderMaterials", "Assets/AllIn1SpriteShader/Materials");
	...
	private void SetNewNormalTexture() {
		...
		if (PlayerPrefs.HasKey("All1ShaderNormals"))
			...
			PlayerPrefs.SetString("All1ShaderNormals", "Assets/AllIn1SpriteShader/Textures/NormalMaps");
		if (!System.IO.Directory.Exists(path)) {
	...
	private void SetNewNormalTexture3() {
		...
		if (isSpriteRenderer)
			normalM = CreateNormalMap(normalMapSr.sprite.texture, normalStrenght, normalSmoothing);
		else
			normalM = CreateNormalMap(mainTex2D, normalStrenght, normalSmoothing);
